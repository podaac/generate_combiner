...
# Program to combine the 2 or 3 files;
#
# MODIS Level 2:  SST/SST4/OC
# VIIRS Level 2:  SST/SST3/OC
#
# The SST4/SST3 are for Night observation and the Ocean Color (OC) is for Day observation.
#
# Input (required):
#
#      perl generic_level2_combiner.pl -data_source=MODIS_A -processing_type=AQUA_QUICKLOOK  -threshold_to_wait=35
#      perl generic_level2_combiner.pl -data_source=MODIS_T -processing_type=TERRA_QUICKLOOK -threshold_to_wait=35
#      perl generic_level2_combiner.pl -data_source=MODIS_A -processing_type=AQUA_REFINED    -threshold_to_wait=35
#      perl generic_level2_combiner.pl -data_source=MODIS_T -processing_type=TERRA_REFINED   -threshold_to_wait=35
#      perl generic_level2_combiner.pl -data_source=VIIRS   -processing_type=VIIRS_QUICKLOOK -threshold_to_wait=35
#      perl generic_level2_combiner.pl -data_source=VIIRS   -processing_type=VIIRS_REFINED   -threshold_to_wait=35
#
#   where    data_source = {MODIS_A,MODIS_T}
#   and  processing_type = {AQUA_QUICKLOOK,TERRA_QUICKLOOK,AQUA_REFINED,TERRA_REFINED}
#   and threshold_to_wait is how long to wait for the SST3 or OC to show up if they are not there already.
#
# Input (optional):
#
#       -max_files=10
...
# Complete examples:
#
#      perl generic_level2_combiner.pl -data_source=MODIS_A -processing_type=AQUA_QUICKLOOK  -threshold_to_wait=35 -max_files=1  -perform_move_instead_of_copy=no
#      perl generic_level2_combiner.pl -data_source=MODIS_A -processing_type=AQUA_QUICKLOOK  -threshold_to_wait=35 -max_files=10 -perform_move_instead_of_copy=yes
#      perl generic_level2_combiner.pl -data_source=MODIS_T -processing_type=TERRA_QUICKLOOK -threshold_to_wait=35 -max_files=10 -perform_move_instead_of_copy=yes
#      perl generic_level2_combiner.pl -data_source=MODIS_A -processing_type=AQUA_REFINED    -threshold_to_wait=35 -max_files=10 -perform_move_instead_of_copy=yes
#      perl generic_level2_combiner.pl -data_source=MODIS_T -processing_type=TERRA_REFINED   -threshold_to_wait=35 -max_files=10 -perform_move_instead_of_copy=yes
#      perl generic_level2_combiner.pl -data_source=VIIRS  -processing_type=VIIRS_REFINED   -threshold_to_wait=0  -max_files=1  -perform_move_instead_of_copy=no -skip_additional_day=yes
#      perl generic_level2_combiner.pl -data_source=VIIRS  -processing_type=VIIRS_QUICKLOOK -threshold_to_wait=0  -max_files=1  -perform_move_instead_of_copy=no -skip_additional_day=yes
#
# Some notes:
#
...
#     2) The Day file has no matching SST3, only the Night file.  This script will check to see if the file is Day or Night.
#        If it is Day, a dummy name will be added to the matching pair SST/SST3.  The IDL code will be able to recognize if it is a real file.
...
#       5 - Cleaning
#
#    When the program start, it will crawl the given directory for a list of SST/SST3 file names.
#    It is assume that these files would have been uncompresssed already by another script.
#    If the SST is a Night file and an SST3 can be found for the same time frame as the SST
#    then the pair is qualified to be combined.  If the SST3 is not found, the holding tank will be checked.  If it is not found there neither, the age of the SST
#    will be checked to see if it passes a certain threshold.  If the threshold has passed, the SST will be combined without an SST3.
#
#    If the SST is a Night file, the SST3 will be combined.  If the SST is a Day file, the OC file will be combined if it is available.
#    If the SST is a Mixed file, it will be combine with the SST3 to create a Mixed file.
...
#       - The SST3 will be moved to a holding tank.
...
#    % perl generic_level2_combiner.pl -data_source=MODIS_A -processing_type=AQUA_QUICKLOOK -threshold_to_wait=35 -max_files=1 -perform_move_instead_of_copy=no -test_only=true
#
...
#
#    and that the directory /data/dev/scratch/IO/modis/MODIS_AQUA_L2_SST_OBPG_QUICKLOOK contains at least 1 SST and either an SST3 (for night file) or OC file (for day file).
#
...
#    An ERROR sigevent will be raised, and the SST/SST3/OC files will be moved to $SCRATCH_AREA/quarantine directory instead of deleting it.
#
#------------------------------------------------------------------------------------------------
...
do "$GHRSST_PERL_LIB_DIRECTORY/create_combined_file_final_location.pl";
...
# Define user-defined package.
# This package generic_combiner_job_manager has some useful functions to retrieve tokens for night file name, name extension to expect for input file and which combiner
# to call for which dataset.
use FindBin;    # NET edit. (Locate module.)
use lib "$FindBin::Bin/.";    # NET edit.
use generic_combiner_job_manager;

# Clobal variable(s)

$NUM_FILES_TO_PROCESS = 999; # Just some default number of files to process if none specified at command line.
$g_routine_name = "generic_level2_combiner";
$g_debug = 0;
$g_holding_tank_move_flag = 1;  # This flag controls whether the SST3 and OC will be removed once they have been combined.  A value of 1 will do the move.  A value of 0 will caused the files to be removed from disk.
my $debug_module = "generic_level2_combiner:";
my $debug_mode   = 0;
...
if ((($data_source_flag eq '') || ($processing_type_flag eq ''))  ||
    ...
    print "generic_level2_combiner:ERROR, All parameters must be provided\n";
    print "Variables:\n\n";
    ...
    print "Usage:\n";
    print "\n";
    print "    perl generic_level2_combiner.pl -data_source=VIIRS -processing_type=VIIRS_QUICKLOOK  -max_files=10 -threshold_to_wait=35 -perform_move_instead_of_copy=yes skip_additional_day=yes\n";
    print "    perl generic_level2_combiner.pl -data_source=VIIRS -processing_type=VIIRS_QUICKLOOK  -max_files=1  -threshold_to_wait=0  -perform_move_instead_of_copy=no  skip_additional_day=yes\n";
    print "\n";
    exit(0);
}
...
if ($perform_move_instead_of_copy_flag eq "yes" || $perform_move_instead_of_copy_flag eq "no") {
   ...
   die("generic_level2_combiner:ERROR: The values of perform_move_instead_of_copy must be yes or no");
}

# Do a sanity check on the value of the threshold to wait.

if ($threshold_to_wait_flag < 0) {
   die("generic_level2_combiner:ERROR: The values of threshold_to_wait_flag must be a positive number");
}
...
my $i_data_source       = uc($data_source_flag);      # The data sources are: {MODIS_A,MODIS_T}
...
$g_gcjm = new generic_combiner_job_manager($i_data_source,$i_processing_type);


# Get the directory to crawl and prefix to look for.

my ($granule_search_directory,$granule_data_name_prefix) = $g_gcjm->get_crawl_info($i_processing_type);

# Do a sanity check on the name of the file for 2019 OBPG naming pattern.
# Then override the value of granule_data_name_prefix to "SNPP_VIIRS".
if ($ENV{GHRSST_OBPG_USE_2019_NAMING_PATTERN} eq "true") {
    $granule_data_name_prefix = "SNPP_VIIRS";
}
...
log_this("INFO",$g_routine_name,"CRAWL_DURATION " . $time_spent_in_crawling . " CRAWL_DIRECTORY " . $granule_search_directory); 
...
if (scalar(@filtered_sst_sst4_names_without_md5_files) == 0) {
    log_this("INFO",$g_routine_name,"NO_FILES_FOUND No SST/SST4/OC files found in $granule_search_directory");
...
if ($g_debug) {
    ...
    $sigevent_msg = "Generic Level 2 Combiner starting for $i_processing_type.";
...
if ($num_sst_sst4_files == 0) {
    my $end_processing_time = localtime;
    log_this("INFO",$g_routine_name,"NONE_QUALIFIED $granule_search_directory");
    log_this("INFO",$g_routine_name,"BEGIN_PROCESSING_TIME $begin_processing_time");
    log_this("INFO",$g_routine_name,"END_PROCESSING_TIME   $end_processing_time");
    my $job_delete_status = delete_job($g_routine_name,$i_processing_type,$job_name);
    exit(0);
...
while (($index_to_sst_sst4_list < $num_sst_sst4_files) && ($num_combined_files_created < $NUM_FILES_TO_PROCESS)) {
  ...
  # Skip if the name contains some sort of .lck since this is not a workable file.
  if (index($working_on_this_file, ".lck") >= 0) {
      $index_to_sst_sst4_list = $index_to_sst_sst4_list + 1;
      next;
  }

  # Check to see if the output file exist in the final location.
  ($o_combined_day_file_final_location,$o_combined_night_file_final_location) = create_combined_file_final_location($working_on_this_file,
                                                                                                                    $output_directory,
                                                                                                                    $i_processing_type);
  # If the output file already exist and the environmental variable GHRSST_MODIS_L2_SKIP_EXISTING_COMBINED_FILE is set to true, we skip this file.
  if ((-e $o_combined_day_file_final_location or -e $o_combined_night_file_final_location) and 
      ($ENV{GHRSST_MODIS_L2_SKIP_EXISTING_COMBINED_FILE} eq "true")) {
     if (-e $o_combined_day_file_final_location) {
         $log_message = "SKIPPING_EXISTING_COMBINED_FILE $o_combined_day_file_final_location working_on_this_file $working_on_this_file index_to_sst_sst4_list $index_to_sst_sst4_list";
     }
     if (-e $o_combined_night_file_final_location) {
         $log_message = "SKIPPING_EXISTING_COMBINED_FILE $o_combined_night_file_final_location working_on_this_file $working_on_this_file index_to_sst_sst4_list $index_to_sst_sst4_list";
     }
     log_this("INFO",$g_routine_name,$log_message);
     $num_combined_files_created = $num_combined_files_created + 1; # Index this even though a file was not actually created.   Just for accounting so the while loop knows when to quit.
  ...
  if ($o_proceed_flag == 0) {
    $index_to_sst_sst4_list = $index_to_sst_sst4_list + 1;  # Index this so we can skip to next name.
    $sigevent_msg = "SST_PROCEED_FLAG_FALSE: file has not passed threshold time: working_on_this_file [$working_on_this_file] o_file_age [$o_file_age] i_threshold_to_wait [$i_threshold_to_wait] o_night_or_day [$o_night_or_day]";
  ...
  if ($o_proceed_flag == 1) {
    $sigevent_msg = "SST_PROCEED_FLAG_TRUE: file has indeed passed threshold time (or files available): working_on_this_file [$working_on_this_file] o_file_age [$o_file_age] i_threshold_to_wait [$i_threshold_to_wait] o_night_or_day [$o_night_or_day]";
    log_this("INFO",$g_routine_name,$sigevent_msg);
  }

  # If we got to here, the output file does not already exist, we can proceed with the combine job.
  ...
  if (!defined $child_pid) {
   ...
   if ($debug_mode) {
       print $debug_module . "night_or_day                 $night_or_day\n";
       print $debug_module . "name_only                    $name_only\n";
       print $debug_module . "i_out_filename               $i_out_filename\n";
       print $debug_module . "upper_level_output_directory $upper_level_output_directory\n";
       print $debug_module . "file_extension               $file_extension\n";
  ...
  for ($loop_count = 1; $loop_count <= $number_of_loops_to_run; $loop_count++) {
    ...
    my $file_extension_token = $g_gcjm->get_name_extension($i_data_source);
    ...
    if ($loop_count == 1) {
        ...
        if (($night_or_day eq 'M') || ($night_or_day eq 'N')) {
            ...
            $file_extension = "." . $file_extension_token . "_GSSTD";
        }
        ...
        $i_sst4_filename = "DUMMY_" . $night_file_token . "_FILENAME";
    ...
    if ((index($i_sst_filename,".nc") >= 0) || ($ENV{GHRSST_PROCESSING_NETCDF_FILE} eq "true")) {
        # Do a sanity check on the name of the file for 2019 OBPG naming pattern.
        #my $name_only = basename($i_sst_filename);
        if (index($name_only,".L2_SNPP_SST.nc") >= 0) {
            #$name_without_extension = basename($name_only,".L2_LAC_SST.nc"); # Get the name without the extension.
            $name_without_extension = basename($name_only,".L2_" . $file_extension_token . "_SST.nc"); # Get the name without the extension.
            if (index($file_extension,".SNPP_GSSTN") >= 0) {
                $name_only = $name_without_extension . ".SNPP_GSSTN.nc";  # 2019176235400.SNPP_GSSTN.nc
            }
            if (index($file_extension,".SNPP_GSSTD") >= 0) {
                $name_only = $name_without_extension . ".SNPP_GSSTD.nc";  # 2019176235400.SNPP_GSSTD.nc
            }
            log_this("DEBUG",$g_routine_name,"name_only[$name_only]");
            log_this("DEBUG",$g_routine_name,"name_without_extension[$name_without_extension]");
            log_this("DEBUG",$g_routine_name,"file_extension[$file_extension]");
            log_this("DEBUG",$g_routine_name,"i_out_filename[$i_out_filename]");
#    log_this("DEBUG",$g_routine_name,"early#exit#0011");
#    exit;
        } elsif (index($name_only,".L2.SST.nc") >= 0)  {
            # We want to get to this V2019176235400.SNPP_GSSTN.nc from SNPP_VIIRS.2019176235400.L2.SST.nc
            if (index($name_only,'VIIRS') >= 0) {
                my $pos_first_dot  = index($name_only,".");
                my $pos_second_dot = index($name_only,".",$pos_first_dot+1);
                $name_without_extension = 'V' . substr($name_only,$pos_first_dot+1,$pos_second_dot-$pos_first_dot-1);
                # Also fix the file_extension from file_extension[._GSSTN] to ".SNPP_GSSTN"
                if (index($file_extension,"._") >= 0) {
                    $file_extension = "." . $file_extension_token . substr($file_extension,index($file_extension,"._")+1) . ".nc";
                }
            } else {
                $name_without_extension = basename($name_only,".L2.SST.nc"); # Get the name without the extension.
            }
            log_this("DEBUG",$g_routine_name,"#0010:name_without_extension[$name_without_extension]");
        }
        #$name_without_extension = basename($name_only,".L2_" . $file_extension_token . "_SST.nc"); # Get the name without the extension.
    } else {
        $name_without_extension = basename($name_only,".L2_" . $file_extension_token . "_SST"); # Get the name without the extension.
    }
    ...
    log_this("DEBUG",$g_routine_name,"name_only[$name_only]");
    log_this("DEBUG",$g_routine_name,"name_without_extension[$name_without_extension]");
    log_this("DEBUG",$g_routine_name,"file_extension_token[$file_extension_token]");
    log_this("DEBUG",$g_routine_name,"file_extension[$file_extension]");
    log_this("DEBUG",$g_routine_name,"i_out_filename[$i_out_filename]");
    log_this("DEBUG",$g_routine_name,"actual_output_fullfilename[$actual_output_fullfilename]");
...
#print "pre:move_combined_file_to_final_location output_directory               $output_directory\n";
#print "pre:move_combined_file_to_final_location i_processing_type              $i_processing_type\n";
#print "pre:move_combined_file_to_final_location name_only                      $name_only\n";
#print "pre:move_combined_file_to_final_location o_actual_file_to_look          $o_actual_file_to_look\n";
#print "pre:move_combined_file_to_final_location o_output_file_name_wild_search $o_output_file_name_wild_search\n";
#print "pre:move_combined_file_to_final_location sigevent_email_to              $sigevent_email_to\n";
#print "pre:move_combined_file_to_final_location sigevent_clause                $sigevent_clause\n";
#print "pre:move_combined_file_to_final_location temp_dir                       $temp_dir\n";
#print "pre:move_combined_file_to_final_location msg2report                     $msg2report\n";
#exit(0);
...
if ($g_debug) {
    $sigevent_type = "information";
    $sigevent_msg = "Generic Level 2 Combiner ended.";
    $sigevent_data = ""; # Must reset to empty string to signify there's no data to pass along.
    ghrsst_notify_operator($g_routine_name,$sigevent_type,$sigevent_msg,$sigevent_email_to,$sigevent_clause,$temp_dir,$msg2report,$sigevent_data);
}
...
#------------------------------------------------------------------------------------------------------------------------
sub get_matching_night_filename_historical {
    ...
    my $night_file_token = $g_gcjm->get_night_file_token_in_name($i_data_source);
    my $replace_with = "$night_file_token" . ".nc";             # Replace with this string.
    $find            = quotemeta $find;                        # Escape regex metachars if present.
    $i_sst_filename_compressed_file =~ s/$find/$replace_with/g;  # Do the replacement of SST.nc to SST4.nc
...
#exit(0);
    ...
    if ($status == 1) { # If cannot decompressed, move on to the next pair.
        ...
        $o_sst4_filename = "DUMMY_" . $night_file_token . "_FILENAME";
        $sigevent_type = "error";
    ...
    my ($oc_name_in_list_flag,$o_index_found_to_sst4_filenames)   = is_oc_name_in_list($i_sst_filename,\@filtered_sst_sst4_names_without_md5_files);
    ...
    if ($i_oc_filename eq "DUMMY_OC_FILENAME") {
        my ($r_matching_sst4_name,$r_matching_oc_name) = get_names_from_holding_tank($i_sst_filename,$scratch_area);
        # If the named of the LAC_OC returned is not empty, we save it.
...
sub verify_combined_file_is_created {
    ...
    my $g_routine_name = 'generic_level2_combiner:verify_combined_file_is_created:';
    ...
    $pos_of_dot = index($i_name_only,".L2_LAC_SST.nc");
    if ($pos_of_dot > 0) {
...
# Change from SNPP_VIIRS.2019176235400.L2.SST.nc to V2019176235400.L2.SST.nc
	   ...
	   if ($debug_flag) {
        ...
        # For some strange reason, the actual file name is V2019176235400.L2_LAC_SST.nc.SNPP_GSSTN.nc
        # We have to check for SNPP_GSSTN.nc or SNPP_GSSTD.nc
        if (index($i_name_only,"SNPP_GSSTN.nc") >= 0) {
            $o_output_file_name_wild_search = $i_upper_level_output_directory . "/" . $i_name_only;
        } elsif (index($i_name_only,"SNPP_GSSTD.nc") >= 0) {
            $o_output_file_name_wild_search = $i_upper_level_output_directory . "/" . $i_name_only;
        } else {
            log_this("ERROR",$g_routine_name,"#0006:Expecting i_name_only to contain SNPP_GSSTN.nc or SNPP_GSSTD.nc " . $i_name_only);
	    if ($debug_flag) {
		log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:i_name_only[$i_name_only]");
		log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:i_file_extension [$i_file_extension]");
		log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:exit#early#0099");
	    }
            exit(0);
        }

        #$o_output_file_name_wild_search = $i_upper_level_output_directory . "/" . $name_without_extension . "*";
        #$o_output_file_name_wild_search = $i_upper_level_output_directory . "/" . $name_without_extension . $i_file_extension . "*";
	if ($debug_flag) {
	    log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:i_name_only[$i_name_only]");
	    log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:pos_of_dot[$pos_of_dot]");
	    log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:name_without_extension[$name_without_extension]");
	    log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:i_file_extension [$i_file_extension]");
	    log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:o_output_file_name_wild_search[$o_output_file_name_wild_search]");
	}
    }
...
#print "verify_combined_file_is_created:i_name_only                      $i_name_only\n";
#print "verify_combined_file_is_created:i_file_extension                 $i_file_extension\n";
#print "verify_combined_file_is_created:i_upper_level_output_directory   $i_upper_level_output_directory\n";
#print "verify_combined_file_is_created:pos_of_dot                       $pos_of_dot\n";
#print "verify_combined_file_is_created:name_without_extension           $name_without_extension\n";
#print "verify_combined_file_is_created:o_file_created_successfully_flag $o_file_created_successfully_flag\n";
#print "verify_combined_file_is_created:o_actual_file_to_look            $o_actual_file_to_look\n";
#print "verify_combined_file_is_created:o_output_file_name_wild_search   $o_output_file_name_wild_search\n";

    if ($debug_flag) {
	log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:o_actual_file_to_look[$o_actual_file_to_look]");
	log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:o_output_file_name_wild_search[$o_output_file_name_wild_search]");
    }
    #log_this("DEBUG",$g_routine_name,"#0006:VARIABLE_CHECK:early#exit#0001");
    #exit;
...
#print "idl_argument_strings [$idl_argument_strings]\n";
#return(1);
    ...
    if ((index($i_sst_filename,".nc") >= 0) || ($ENV{GHRSST_PROCESSING_NETCDF_FILE} eq "true")) {
        if ($ENV{GHRSST_MODIS_L2_COMBINER_USE_NETCDF_COMBINER_TO_HDF} eq "true") {
            ...
            my $idl_combiner_program_name = $g_gcjm->setup_idl_combiner_program_name($i_data_source);
            #$rt_flag = "-quiet -rt=$GHRSST_IDL_LIB_DIRECTORY/combine_netcdf_sst_and_sst4_files_to_netcdf.sav";
            $rt_flag = "-quiet -rt=$GHRSST_IDL_LIB_DIRECTORY/" . $idl_combiner_program_name;
            if ($g_debug) {
...
sub perform_decompression_task_historical {
    ...
    my $debug_module = "perform_decompression_task_historical:";
    my $debug_mode   = 0;

    # If the name is a dummy name, we merely returns the original name.
    my $night_file_token = $g_gcjm->get_night_file_token_in_name($i_data_source);
    my $dummy_night_file_name  = "DUMMY_" . $night_file_token . "_FILENAME";
    if ($i_compressed_filename eq $dummy_night_file_name) { 
    ...
    if ($debug_mode) {
        ...
        print $debug_module . "o_uncompressed_filename [$o_uncompressed_filename]\n";
    }
...
#exit(0);
    ...
    return ($o_oc_name_in_list_flag,$o_index_found_to_filenames);
}
...
sub prepare_output_work_area_with_error_handling_historical {
    ...
    my @name_split_tokens = split(/\./,$o_name_only);   # Split into V2016001000000 L2_SNPP_SST nc
    ...
    if ($when_granule_observed_flag eq 'Day') {
        $o_night_or_day = 'D';
        $o_file_extension = "." . $sensor_name . "_GSSTD";   # To get .SNPP_GSSTD
    }
    if ($when_granule_observed_flag eq 'Night') {
        $o_night_or_day = 'N';
        $o_file_extension = "." . $sensor_name . "_GSSTN";   # To get .SNPP_GSSTN
    }
...
sub proceed_with_processing_sst_file {
    ...
    my $g_routine_name = 'generic_level2_combiner:proceed_with_processing_sst_file:'; 
    ...
    if (-e $i_sst_filename) {
        ...
        log_this("ERROR",$g_routine_name,"File not found $i_sst_filename");
        exit(1);
    }
    ...
    log_this("INFO",$g_routine_name,"i_sst_filename,i_sst4_filename,night_or_day,file_age,threshold_to_wait,_proceed_flag [$i_sst_filename] [$i_sst4_filename] [$o_night_or_day] [$o_file_age] [$i_threshold_to_wait] [$o_proceed_flag]");
